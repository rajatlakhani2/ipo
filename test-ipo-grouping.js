...
// Direct implementation with functions included
...
function testApplicationDisplayCompleteness() {
    ...
    for (let iteration = 0; iteration < 100; iteration++) {
        ...
        // Mock the rendering function behavior
        // In a real implementation, this would test the actual HTML rendering
        const requiredFields = ['investor_name', 'application_amount', 'bank_name', 'status', 'payment_status', 'profit'];
        
        // Simulate what the rendered output should contain
...
function testInvestorFieldPrefilling() {
    ...
    for (let iteration = 0; iteration < 100; iteration++) {
        ...
        // In real implementation, this would test the actual modal prefilling
        const mockModalState = {
            selectedInvestorId: investors.length > 0 ? investors[0].id : null
        };
        
        // Property: If investors exist, one should be selected by default
...
function testBankDropdownSynchronization() {
    ...
    for (let iteration = 0; iteration < 100; iteration++) {
        // Generate random investor with banks
        ...
        // Property: Dropdown should contain exactly the banks from the string
...
function testApplicationAmountCalculation() {
    ...
    for (let iteration = 0; iteration < 100; iteration++) {
        ...
        // Property: Calculated amount should equal num_shares × purchase_price_per_share
...
function testAddButtonPresence() {
    ...
    for (let iteration = 0; iteration < 100; iteration++) {
        ...
        // Mock IPO group rendering
        for (const ipo of ipos) {
            const ipoApps = applications.filter(app => app.ipo_id === ipo.id);
            
            if (ipoApps.length > 0) {
                // Mock rendered HTML should contain Add Application button
                ...
                // Property: HTML should contain Add Application button
...
function testIPOPrefillingFromContext() {
    ...
    for (let iteration = 0; iteration < 100; iteration++) {
        ...
        // Mock modal opening with prefilled IPO
        const mockModalState = {
            prefilledIPOId: ipo.id,
            selectedIPOId: ipo.id // Should be set to the prefilled value
        };
        
        // Property: When modal is opened with IPO context, that IPO should be selected
...
// Update the main test runner to include all tests
function runAllPropertyTestsComplete() {
...
// Export the complete test runner
if (typeof module !== 'undefined' && module.exports) {
    module.exports.runAllPropertyTestsComplete = runAllPropertyTestsComplete;
    module.exports.testApplicationDisplayCompleteness = testApplicationDisplayCompleteness;
    module.exports.testInvestorFieldPrefilling = testInvestorFieldPrefilling;
    module.exports.testBankDropdownSynchronization = testBankDropdownSynchronization;
    module.exports.testApplicationAmountCalculation = testApplicationAmountCalculation;
    module.exports.testFormValidationCompleteness = testFormValidationCompleteness;
    module.exports.testAddButtonPresence = testAddButtonPresence;
    module.exports.testIPOPrefillingFromContext = testIPOPrefillingFromContext;
}