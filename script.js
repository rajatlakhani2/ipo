...
// API_BASE is set in api-client.js (loaded first)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        ...
        loadView(typeof loadHomeView === 'function' ? 'home' : (typeof loadCommandBoardView === 'function' ? 'command-board' : 'dashboard'));
    }, 100);
});
...
async function loadDashboardView() {
    ...
    try {
        const [summaryRes, fundingRes] = await Promise.all([
            ...
            apiFetch(`${API_BASE}/reports/funding-summary`)
        ]);
...
async function loadInvestorsView() {
    ...
    try {
        const response = await apiFetch(`${API_BASE}/investors`);
...
async function loadIPOsView() {
    ...
    try {
        const response = await apiFetch(`${API_BASE}/ipos`);
    ...
    document.getElementById('bulk-apply-form').addEventListener('submit', async (e) => {
        ...
        if (confirm(`Are you sure you want to apply for ${selectedInvestorIds.length} investors?`)) {
            ...
            for (const investorId of selectedInvestorIds) {
                try {
                    await apiFetch(`${API_BASE}/applications`, {
                        method: 'POST',
                ...
                <div class="card" style="margin-bottom: 24px;">
                    ...
                    `<span><i class="fas fa-users"></i> ${investors.length} Investors</span>
                        ...
                        <div style="display: flex; align-items: center; gap: 20px;">
                            ...
                            <button class="btn btn-secondary" onclick="openBulkAllotmentModal(${ipo.id}, '${(ipo.ipo_name || '').replace(/'/g, "\\'")}')" title="Bulk allotment import">
                                <i class="fas fa-tasks"></i> Bulk Results
                            </button>
                            <button class="btn btn-secondary" onclick="exportApplications(${ipo.id})" title="Export Excel">
...
async function loadBankWiseView() {
    ...
    try {
        const [appRes, bankStatsRes] = await Promise.all([
            apiFetch(`${API_BASE}/applications`),
            apiFetch(`${API_BASE}/reports/bank-stats`)
        ]);
...
async function loadMoneyTransfersView() {
    ...
    try {
        const response = await apiFetch(`${API_BASE}/money-transfers`);
...
async function loadReportsView() {
    ...
    try {
        const [bankStatsRes, ipoProfitRes] = await Promise.all([
            apiFetch(`${API_BASE}/reports/bank-stats`),
            apiFetch(`${API_BASE}/reports/ipo-profit`)
        ]);
...
async function quickApply(investorId, ipoId, amount, upiId) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            showToast(err.error || 'Failed to apply. Please try again.', 'error');
        }
    } catch (error) {
...
function addApplicationModal() {
    ...
    document.getElementById('application-form').addEventListener('submit', async (e) => {
        ...
        try {
            if (id) {
                await apiFetch(`${API_BASE}/applications/${id}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(data)
                });
...
function addImportModal() {
    ...
    document.getElementById('import-form').addEventListener('submit', async (e) => {
        ...
        try {
            const res = await apiFetch(`${API_BASE}/investors/import`, {
                method: 'POST',
                body: formData
            });
...
async function deleteInvestor(id) {
    if (confirm('Are you sure you want to delete this investor?')) {
        await apiFetch(`${API_BASE}/investors/${id}`, { method: 'DELETE' });
        loadInvestorsView();
    }
}
...
async function deleteIPO(id) {
    if (confirm('Are you sure you want to delete this IPO?')) {
        await apiFetch(`${API_BASE}/ipos/${id}`, { method: 'DELETE' });
        loadIPOsView();
    }
}
...
async function markAsRepaid(id) {
    if (confirm('Mark this transfer as repaid?')) {
        try {
            await apiFetch(`${API_BASE}/money-transfers/${id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
...
async function deleteMoneyTransfer(id) {
    if (confirm('Are you sure you want to delete this money transfer?')) {
        try {
            await apiFetch(`${API_BASE}/money-transfers/${id}`, { method: 'DELETE' });
            loadMoneyTransfersView();
        } catch (error) {
